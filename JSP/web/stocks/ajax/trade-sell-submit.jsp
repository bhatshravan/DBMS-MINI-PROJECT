	...
	if(rs.next())
		{
			...
			if(sale>money)
			{
				...
				else
				{
					...
					ResultSet sp = st.executeQuery("select * from investments WHERE stock='"+request.getParameter("stock")+"' AND user_id = '"+uid+"' AND time='"+date+"';");
					if(sp.next())
					{
					...
					else
					{
						...
						cstat.setString(2, symbol);
						cstat.setString(3, volume);
						cstat.setString(4, bank);
						cstat.setString(5, prices);
						cstat.setString(6, date);
					...
					rs2 = st.executeQuery("select current_money from bank WHERE customer_id="+uid+";");
					if(rs2.next())
					{
